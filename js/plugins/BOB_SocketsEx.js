 ...
 *插槽宝石在防具里设置，防具类型必须设置为无。
注释区添加<SocketEx>，声明这是一个宝石。

也可以限制为用于武器还是护具。
<SocketEx Weapon>
<SocketEx Armor>

也可以具体限制只能给某一装备类型使用（etc为等等，更多的意思）
<SocketEx Weapon: ID, ID, etc>
<SocketEx Armor: ID, ID, etc>

在武器或者护具注释加入<Sockets: X>来设置插槽数x

注意：你需要配合Yanfly's Item Core使用才能正确的产生选择效果
...
SocketsEx.prototype.update = function(owner) {
	for (var i = 0; i < this.sockets.length; i++) {
		this.processRemoval(owner, this.sockets[i]);
		this.sockets[i].removed();
		this.processApply(owner, this.sockets[i]);
		this.sockets[i].inserted();
	}
};
...
SocketsExSocket.prototype.removed = function() {
	this._removed = false;
};

SocketsExSocket.prototype.inserted = function() {
	this._inserted = false;
};
...
Window_SocketExSockets.prototype.updateHelp = function() {
	this._helpWindow.setItem(this.item().item);
};
...
Window_SocketExGems.prototype.makeItemList = function() {
    if (this._itemRef === null) { return false; }
